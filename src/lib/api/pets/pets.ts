...
const pets = async (
  ...
  const res: AxiosResponse = await axios.get(petsPath, {
    params: { page, page_size: pageSize },
  });
  return res.data;
};
export const usePets = (page: number, pageSize: number) =>
  useQuery({
    queryKey: [petsPath, page, pageSize],
    queryFn: () => pets(page, pageSize),
    placeholderData: (p) => p,
  });
